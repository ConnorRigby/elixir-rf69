defmodule RF69 do
  ...
  def handle_info(:init, rf69) do
    {:ok, reset} = GPIO.open(@reset_pin, :output, initial_value: 0)
    {:ok, ss} = GPIO.open(@ss_pin, :output, initial_value: 1)
    {:ok, irq} = GPIO.open(@irq_pin, :input)
    {:ok, spi} = SPI.open(@spi_bus_name, mode: 0, speed_hz: 8_000_000)
    send(self(), :reset)
  ...
  def handle_info(:reset, rf69) do
    ...
    :ok = GPIO.set_interrupts(rf69.irq, :rising)
  ...
  def handle_info({:circuits_gpio, @irq_pin, _, 1}, rf69) do
    <<
      _fifo_full::1,
      _fifo_not_empty::1,
      _fifo_level::1,
  ...
  defp recv_packet(rf69) do
    ...
    {:ok, _} = SPI.transfer(rf69.spi, <<0::8>>)

    # read header
    {:ok, <<length::8, target_id::8, sender_id::8, ctl::bits-8>>} =
      SPI.transfer(rf69.spi, <<0::32>>)
    ...
    {:ok, <<payload::binary-size(length), _::binary-3>>} =
      SPI.transfer(rf69.spi, <<0::size(size)>>)
  ...
  defp send_packet(%Packet{} = packet, rf69) do
    ...
    # select FIFO register
    {:ok, _} = SPI.transfer(rf69.spi, <<0b10000000>>)
    {:ok, _} = SPI.transfer(rf69.spi, send_packet)
    unselect(rf69)
